...
class EventSubDetailPage extends StatefulWidget {
  ...
  // detail screen
  @override
  State createState() => new ScheduleTimeDetailState();
}

class ScheduleTimeDetailState extends State<EventSubDetailPage> {
...
//                                eventGroupChatId: widget.eventDocument.groupID,
//                                endTime: endTime,
//                                eventActivity: newValue,
//                                eventCalendar: widget.eventDocument.calendar,
//                                eventNote: _Note)) {
//                              // if data is updated in database then update the view with new data
//                              setState(() {
//                                // update in ScheduleTimeDetail
...
//          owner: MyAuthentication.authenticatedUser.uid);
    ...
    // Notify user on outcome of save attempt
    // Toast when event is saved
    // Scaffold.of(context) == null because our context contains
    // Scaffold. So we have to give the Scaffold a GlobalKey
    // and use it here.
    // Remove the current SnackBar if there is one
    EventTabView.scaffoldKey.currentState.removeCurrentSnackBar();
    // Display SnackBar with the message
    EventTabView.scaffoldKey.currentState
        .showSnackBar(new SnackBar(content: new Text(messageAboutSaveSuccess)));
...
//        noteToAdd;
  ...
  ///temporary display that notifys user that item is still in development.
  void stillInDevSnackbar() {
    // Remove the current SnackBar if there is one
    EventTabView.scaffoldKey.currentState.removeCurrentSnackBar();
    // Display SnackBar with the message
    EventTabView.scaffoldKey.currentState.showSnackBar(new SnackBar(
        content: new Text(
            "This is still in development and currently does nothing.")));
  }
}
