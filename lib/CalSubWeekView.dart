...
class CalSubScheduleViewPageState extends State<CalSubWeekView> {
  ...
  Widget build(BuildContext context) {
    return CustomScrollView(
      slivers: <Widget>[
        SliverToBoxAdapter(
          child: new EventsGrid(allEvents: FakeData.allUserEvents,),
        )
      ],
    );
  }
...
class DayColumn extends StatelessWidget {
  DayColumn({Key key, this.dayEvents}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    for (MyFBDocuments.EventItem x in dayEvents) {
      ...
      eventItems.add(new EventItem(eventDoc: x,));
...
//  Widget getTextWidgets(List<MyFBDocuments.EventItem> eventItems1)
//  {
//    return new Row(children: eventItems1.map((item) => new EventItem(eventDoc: item)).toList());
//  }
}
